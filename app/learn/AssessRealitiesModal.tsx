...
export default function AssessRealitiesModal({
  visible,
  onClose,
  goalId,
  ...
  const [realitiesAddedSinceLastLoad, setRealitiesAddedSinceLastLoad] =
    useState(false);
  ...
  const handleAddRealityFromQuestion = async (questionIndex: number) => {
    ...
    if (text) {
      try {
        ...
        Alert.alert(
          t("learn.goalDetails.realities.assess.success"),
          newReality.reality
        );
      } finally {
  ...
  return (
    <ModalView
      visible={visible}
      onClose={onClose}
      title={t("learn.goalDetails.realities.assess.title")}
    >
